...
class RandT2(object):
	...
	print('\n\n\n\nDEBUG DOUBLE CHECK-- CLASS RANDT2 STARTS NOW! (debugging):\n')

	def __init__(self):
		self.results = {}

		from sklearn.datasets import fetch_openml
		#iris = fetch_openml(name='iris', version = 'active')
	...
	def turtlesetup2(self, fetch_new2):
		...
		if( str(iris.DESCR).count(str('\n	2. ')) < 1 ):
			...
			while(len(p_features) < 3):
				...
				p_this = input('Enter the number of the FIRST feature you want to use in this 3d model: ')
			...
			for elem1 in list(range( iris.data.shape[1] - 1)):
				...
				selected_feature_s = input('Enter the feature number for any additional features you wish to add: ')
				p_features.append(int(selected_feature_s) - 1)

		print("\nYou have selected the following features: " + str(iris.feature_names[ int(p_features[0]) ]) + str(iris.feature_names[ int(p_features[1]) ]) + str(iris.feature_names[ int(p_features[2]) ]))
...
#		ax1.plot( points[:,p_features[0]], points[:,p_features[1]], points[:,p_features[2]], 'k.', alpha=0.7 )
		...
		iris2 = iris.data[[p_features],:]
		
		print(max(iris.data[14]))

		
		
		ax1.set_xlim(min(iris.data[:, p_features[0]]), max(iris.data[:, p_features[0]]) )
		ax1.set_ylim(min(iris.data[:, p_features[1]]), max(iris.data[:, p_features[1]]) )
		ax1.set_zlim(min(iris.data[:, p_features[2]]), max(iris.data[:, p_features[2]]) )
		...
		print('\n\n\n\nHERE I LAY OUT THE LIMITING VALUES FOR PLOT (debugging):\n')
		print('\n\n\n\nHERE I LAY OUT THE LIMITING VALUES FOR PLOT (debugging):\n')
		print('\n\n\n\nHERE I LAY OUT THE LIMITING VALUES FOR PLOT (debugging):\n')
		
		print('\nxlim values are: ' + str(ax1.set_xlim))
		print('\nylim values are: ' + str(ax1.set_ylim))
		print('\nzlim values are: ' + str(ax1.set_zlim))
		...
		res_out1 = {}
		res_out1['output_decide_regions'] = cs
